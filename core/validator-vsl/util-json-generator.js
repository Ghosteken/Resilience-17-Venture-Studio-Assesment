/* eslint-disable camelcase, no-param-reassign, no-unused-vars */
...
function generateJSONType(AST, trueAST, tabIndexCount = 0) {
  ...
  keys.forEach((k) => {
    ...
    if (nodeHasChildren) {
      typeScriptStringTokens.push(generateJSONType(node.children, trueAST, tabIndexCount + 1));
    }
    if (typeBlockOpened) {
...
module.exports = generateJSONType;
